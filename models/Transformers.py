# Thanks to Vincent Bonus: https://wingedsheep.com/building-a-language-model/
# I used his code for Transformers implementation from scratch
# Just changed the Training Data
...
class Runner(torch.nn.Module):
    ...
    def run(self):
        ...
        padding_token_id = tokenizer.character_to_token('<pad>') 

        print(f"Tokenizer size: {number_of_tokens}")
        ...
        model = AutoregressiveWrapper(LanguageModel(
            ...
            number_of_layers=number_of_layers,
            dropout_rate=dropout_rate,
            max_sequence_length=max_sequence_length # Pass max_sequence_length here
        )).to(get_device())
        print(f"Model created and sent to device: {get_device()}")

        print("Loading and preparing training data...")
        data_file = "all_txts_combined.txt"
        try:
        ...
        print(f"Loaded {len(raw_training_data)} characters from {data_file}")

        processed_training_data = re.sub(r'[^a-z0-9 .]', '', raw_training_data.lower())
        print(f"Processed data length: {len(processed_training_data)} characters")

        if len(processed_training_data) < max_sequence_length + 1:
             raise ValueError(f"Processed data length ({len(processed_training_data)}) is too short for the max_sequence_length ({max_sequence_length}). Need at least {max_sequence_length + 1} characters.")

        tokenized_data = tokenizer.tokenize(processed_training_data) 

        sequences_standard = []
        for i in range(0, len(tokenized_data) - max_sequence_length):
             # Sequence includes input + target token
             seq = tokenized_data[i : i + max_sequence_length + 1]
             sequences_standard.append(seq)
        print(f"Created {len(sequences_standard)} standard sequences.")
        if not sequences_standard:
             raise ValueError("No standard sequences created. Check data/sequence length.")
        sequences_to_train = sequences_standard
        ...
        loss_per_epoch = trainer.train(sequences_to_train, epochs=epochs, batch_size=batch_size)
        ...
        model_save_path = './trump_transformer_model.pth'
        ...
        prompt = "make"
        processed_prompt = re.sub(r'[^a-z0-9 .]', '', prompt.lower())
        ...
        cleaned_generated_text = generated_text.replace('<pad>', '').strip()
        print("\n--- Generated Text ---")
